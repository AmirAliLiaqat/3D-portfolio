...
export const GENERIC_TOPIC_WORDS = [
  "project", "projects", "service", "services", "skill", "skills",
  "experience", "work", "works", "working", "job", "jobs", "career",
  "education", "degree", "study", "studies", "studied", "university",
  "college", "school", "contact", "email", "phone", "whatsapp", "reach",
  "hire", "hiring", "available", "availability", "rate", "rates", "price",
  "pricing", "cost", "budget", "quote", "testimonial", "testimonials",
  "review", "reviews", "client", "clients", "company", "bio", "background",
  "tech", "technology", "technologies", "stack", "framework", "language",
  "languages", "achievement", "achievements", "certificate", "certificates",
  "certification", "certifications", "portfolio", "resume", "cv",
  "freelance", "freelancer", "remote", "location", "based", "github",
  "linkedin", "social", "blog", "blogs", "article", "articles", "name",
  "role", "designation", "expertise", "specialize", "specialise",
  "specialty", "offer", "offers", "build", "built", "building", "develop",
  "developed", "developer", "development", "app", "apps", "application",
  "applications", "website", "websites", "web", "mobile", "amir",
  "designstodeploy", "collaborate", "collaboration", "yourself",
];
...
export const buildPortfolioContext = async () => {
  ...
  const [profile, company, services, skills, experiences, education, projects, testimonials] =
    await Promise.all([
  ...
  if (profile?.description) lines.push(`Summary: ${truncate(profile.description, 500)}`);
  ...
  if (company?.name) {
    ...
    if (company.shortDescription) lines.push(`Company summary: ${truncate(company.shortDescription, 300)}`);
  ...
  if (services?.length) {
    ...
    services.forEach((s) => {
      lines.push(`- ${s.title}${s.category ? ` (${s.category})` : ""}: ${truncate(s.description, 150)}`);
      addKeywords(s.title);
  ...
  if (experiences?.length) {
    ...
    experiences.forEach((e) => {
      ...
      (e.points || []).slice(0, 3).forEach((p) => lines.push(`  * ${truncate(p, 150)}`));
    });
  }
...
export const getGroqReply = async (messages) => {
  ...
  if (!response.ok) {
    ...
    const err = new Error(`Groq API request failed (${response.status}): ${errBody}`);
